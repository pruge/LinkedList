...
template <class T> struct ListNode {
  ...
  ListNode<T>(const T &_v, ListNode<T> *_next = nullptr)
      : data(_v), next(_next) {}
};
...
public:
  ...
  LinkedList(const LinkedList<T> &rhs) : LinkedList() {
    *this = rhs;
  }; // clone constructor
  virtual ~LinkedList();
  ...
  */
   ...
   * i.e. it's OK to clear nodes containing trivial objects like int, float,
   * etc... if node contains pointers to dynamically allocated objects created
  ...
  */
  struct ConstIterator {
    ...
    ConstIterator &operator++() {
      m_ptr = m_next_ptr;
      m_next_ptr = m_next_ptr ? m_next_ptr->next : nullptr;
      return *this;
    }
    ...
    // return a.m_ptr == b.m_ptr; }; friend bool operator!= (const
    // ConstIterator& a, const ConstIterator& b) { return a.m_ptr != b.m_ptr; };
  ...
  /*
          Iterator class
          inherits from ConstIterator, provides mutable forward iterator for the
     list
  */
  ...
  } // same as last->next for non-empty list
...
template <typename T> unsigned LinkedList<T>::size() const { return _size; }

template <typename T> LinkedList<T>::LinkedList(unsigned sizeIndex, T _t) {
  for (unsigned i = 0; i < sizeIndex; i++) {
    add(_t);
  }
}

template <typename T> bool LinkedList<T>::add(unsigned index, const T &_t) {
  if (index >= _size)
    return add(_t);
...
template <typename T> bool LinkedList<T>::add(const T &_t) {
...
template <typename T> bool LinkedList<T>::unshift(const T &_t) {
...
template <typename T> bool LinkedList<T>::set(unsigned index, const T &_t) {
...
template <typename T> void LinkedList<T>::_pop() {
  if (!_size)
    return;
...
template <typename T> void LinkedList<T>::_shift() {
  if (!_size)
    return;
...
template <typename T> T LinkedList<T>::remove(unsigned index) {
  if (index >= _size)
    return T();
...
template <typename T> void LinkedList<T>::unlink(unsigned index) {
  if (!_size || index >= _size)
    return;
...
template <typename T> T LinkedList<T>::get(unsigned index) const {
...
template <typename T> bool LinkedList<T>::exist(unsigned index) const {
  return (index < _size);
}
...
LinkedList<T> &LinkedList<T>::operator=(const LinkedList<T> &rhs) {
  ...
  // for (const auto& i :  std::as_const(rhs))		// std::as_const req
  // c++17 	add(*i);
